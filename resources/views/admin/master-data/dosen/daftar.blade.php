<x-app-layout>
    <div class="space-y-5">
        <div>
            <h1 class="text-2xl font-extrabold tracking-normal text-slate-950">Data Dosen</h1>
            <p class="mt-1 text-sm font-medium text-slate-500">Kelola data dosen dan informasi fingerprint terdaftar.</p>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <div class="text-sm font-semibold text-slate-500">Total Dosen</div>
                        <div class="mt-1 text-3xl font-extrabold text-slate-950">{{ number_format($stats['total'], 0, ',', '.') }}</div>
                        <div class="mt-2 text-xs font-medium text-slate-500">Seluruh dosen terdaftar</div>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <div class="text-sm font-semibold text-slate-500">Dosen Aktif</div>
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 p-5">
                <form method="GET" class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                    <div class="grid flex-1 gap-3 md:grid-cols-[minmax(220px,1fr)_180px]">
                        <label class="relative">
                            ...
                            <input
                                ...
                                placeholder="Cari dosen, NIDN, email, atau fingerprint ID..."
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('admin.lecturers.import') }}" class="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-emerald-200 px-4 text-sm font-extrabold text-emerald-700 hover:bg-emerald-50">
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            ...
            <div class="overflow-x-auto px-5 pt-5">
                <table class="min-w-full overflow-hidden rounded-lg border border-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-5 py-4 text-left text-xs font-extrabold uppercase tracking-wide text-slate-500">NIDN</th>
                            <th class="px-5 py-4 text-left text-xs font-extrabold uppercase tracking-wide text-slate-500">Nama</th>
                            <th class="px-5 py-4 text-left text-xs font-extrabold uppercase tracking-wide text-slate-500">Email</th>
                            <th class="px-5 py-4 text-left text-xs font-extrabold uppercase tracking-wide text-slate-500">Telepon</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($items as $item)
                            ...
                            <tr class="transition hover:bg-slate-50/70">
                                <td class="whitespace-nowrap px-5 py-4 text-sm font-medium text-slate-600">{{ $item->nidn ?: '-' }}</td>
                                <td class="whitespace-nowrap px-5 py-4 text-sm font-semibold text-slate-800">{{ $item->name }}</td>
                                <td class="whitespace-nowrap px-5 py-4 text-sm font-medium text-slate-600">{{ $item->email ?: '-' }}</td>
                                <td class="whitespace-nowrap px-5 py-4 text-sm font-medium text-slate-500">{{ $item->phone ?: '-' }}</td>
                                ...
                                <td class="whitespace-nowrap px-5 py-4">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route($routePrefix.'.destroy', $item) }}" onsubmit="return confirm('Yakin ingin menonaktifkan dosen ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-sm font-medium text-slate-500">Data dosen belum tersedia.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
